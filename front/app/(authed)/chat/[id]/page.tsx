...
import { findOneChat,} from "@/app/actions/chat";
...
export default function ChatPage() {
  ...
  return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {messages.map(msg => (
              <div
                  key={msg.id}
                  className={`flex ${
                      msg.author.id === chat.participants.find(p => p.id === msg.author.id)?.id
                          ? 'justify-end'
                          : 'justify-start'
                  }`}
              >
        ...
        <MessageInput value={input} onChange={e => setInput(e.target.value)} onSend={handleSend} />
      </div>
  );
}